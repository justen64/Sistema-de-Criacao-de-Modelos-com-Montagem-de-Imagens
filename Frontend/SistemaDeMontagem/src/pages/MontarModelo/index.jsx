...
export const MontarModelo = () => {
  ...
  const columns = [
    {
      ...
      header: "",
      body: (rowData) => {
        const isSelected = Object.values(itensSelecionados).some(
          (item) => item.id === rowData.id
        );

        return (
          <div
            onClick={() => handleSelecionar(rowData)}
            style={{ cursor: "pointer" }}
          >
            <TriStateCheckbox
              value={isSelected ? true : null}
              onChange={() => {}}
              readOnly
              className="p-mr-2"
            />
    ...
    },
    {
      ...
      body: (rowData) => (
        <span>
          {`${rowData.nomeModelo}-${rowData.tipoRecorte}-${rowData.material}-${rowData.corMaterial}`
            .toLowerCase()
            .replace(/\s/g, "_")}
        </span>
      ),
      ...
      >
        ...
        <Button className={styles.teste} onClick={buscar}>
          <i
            ...
            style={{ color: "white", alignSelf: "center", width: "50%" }}
          />
        </Button>
      </span>
    </div>
  ...
  const handleSelecionar = (rowData) => {
    const ordem = rowData.ordemExibicao;

    if (![1, 2, 3].includes(ordem)) {
      toast.error(
        "Só é possível selecionar itens com ordem de exibição 1, 2 ou 3."
      );
      return;
    }

    setItensSelecionados((prev) => {
      if (prev[ordem]?.id === rowData.id) {
        const newState = { ...prev };
        delete newState[ordem];
        return newState;
      }
      return { ...prev, [ordem]: rowData };
    });
  };
  ...
  const handleMontarPeca = () => {
    const selecionados = Object.values(itensSelecionados);

    const temOrdem1 = selecionados.some((item) => item.ordemExibicao === 1);
    const temOrdem2 = selecionados.some((item) => item.ordemExibicao === 2);
    const temOrdem3 = selecionados.some((item) => item.ordemExibicao === 3);

    if (!temOrdem1 || !temOrdem2 || !temOrdem3) {
      toast.error(
        "Você deve selecionar exatamente uma peça com ordem 1, 2 e 3."
      );
      return;
    }

    const ordenado = selecionados
      .filter((item) => [1, 2, 3].includes(item.ordemExibicao))
      .sort((a, b) => a.ordemExibicao - b.ordemExibicao);

    navigate("/montagemCompleta", { state: { camadas: ordenado } });
  };
        ...
        <div className={styles.containerTabela}>
          ...
          <div className={styles.tabela}>
            <GenericTable
              ...
              filters={{ globalFilter, itensSelecionados }}
              header={header}
          ...
          <div className={styles.containerGerarImagem}>
            <Botao
              texto={"Gerar imagem"}
              className={styles.botao}
              onClick={handleMontarPeca}
            />
          </div>
        </div>
      </div>
